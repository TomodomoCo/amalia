...
/*
    ...
    Further modifications by Peter Upfold that are Amalia-specific.
    This includes disabling of remote images, and maximum size of
    original file conditional thumbnail generation.
...
{
	echo 'Show thumbnails is disabled.';
	die();
}
...
// PETERS' EDITS
$cfgPath         = preg_replace("/[^0-9]+/", "", get_request("cfgp", 0));
// END PETER'S EDITS
...
$cache_dir = $config['config_path'].'/userfiles/thumbcache';
...
function open_image($mime_type, $src) {
	...
	// PETER'S EDITS
	// if the file is > 4MB, don't bother thumbnailing
	// as it's too much resource for the trouble
	
	// 4 MB chosen based on my testing of what worked and what
	// crapped out
	
	if (filesize($src) > 2 * 1024 * 1024) // first number is megabytes
	{
		displayError('File is larger than agreed maximum size for thumbnail generation');
		die();
	}
...
function mime_type($file) {
    ...
    if (stristr(PHP_OS, 'WIN') /* PETER'S EDITS */ && strtolower(PHP_OS) != 'darwin') /* END PETER'S EDITS */ { 
        $os = 'WIN';
    } else { 
        $os = PHP_OS;
    }
    ...
    if (function_exists('mime_content_type')) {
        ...
        // PETER'S EDITS
        // strip charset information from mimetype, it is not helpful with images
    ...
    if (!valid_src_mime_type($mime_type) && $os != "WIN") {
        if (preg_match("/FREEBSD|LINUX|DARWIN/", $os)) {
...
function checkExternal ($src) {
    ...
    //PETER patching to remove feature
	$isAllowedSite = false;
	displayError('remote host not allowed');
	return $src;
	// END PETER patching
...
function cleanSource($src) {
    ...
    // HACK to support amalia site path
   	//OLD $src = get_document_root($src) . '/' . $src;
   	if ($cfgPath)
   		$src = $config['config_path'] . '/' . $src;
   	else
	   	$src = $config['site_path'] . '/' . $src;
...
function displayError($errorString = '') {
    ...
    // should anything go wrong, display a pretty picture and log the error message instead
    
    $logFile = get_cache_file();
    
    $logFile = $cache_dir . '/' . $logFile . '.txt';
    
    $log = "Trying to run TimThumb: ".$_SERVER['QUERY_STRING'] . PHP_EOL;
    $log .= "error was: " . PHP_EOL;
    $log .= $errorString . PHP_EOL;
    $log .= __FILE__ . " at ".date('Y-m-d H:i:s') . PHP_EOL;
    ...
    // this is a base64 encoded png file of the generic image filetype icon, at 100x100 pixels
    // encoded here for the purposes of keeping it inside this script and so not to rely on
    // anything else
    $failimage = '
WAcF0xUclQ+wuUv+/91bdgEGAPnlBOUia4UYAAAAAElFTkSuQmCC';
    
    echo base64_decode($failimage);
